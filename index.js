...
function getOpenVaultData() {
    ...
    // FIX: Add null check for context to prevent crashes before chat loads
    if (!context) {
 ...
 */
function showToast(type, message, title = 'OpenVault', options = {}) {
...
function refreshStats() {
    ...
    // FIX: Handle null data (no context available)
    if (!data) {
...
async function onBeforeGeneration(type, options, dryRun = false) {
    ...
    // FIX: Also skip if retrieval already in progress
    if (operationState.retrievalInProgress) {
        log('Skipping retrieval - retrieval already in progress');
        return;
    }

    // FIX: Set retrieval flag immediately to prevent concurrent retrievals
    ...
    try {
        ...
        // FIX: Always clear retrieval flag
...
function updateEventListeners(skipInitialization = false) {
    ...
    // FIX: Always register CHAT_CHANGED handler for UI refresh (regardless of enabled state)
    // This ensures memoryBrowserPage reset and UI refresh happen even when disabled
 ...
 * Clears injection, refreshes UI, and sets cooldown to prevent extraction during chat load
 * This is the single handler for CHAT_CHANGED - registered in updateEventListeners()
 */
function onChatChanged() {
    log('Chat changed, clearing injection and setting load cooldown');

    // FIX: Reset memoryBrowserPage to prevent showing wrong page after chat switch
    memoryBrowserPage = 0;
    ...
    // FIX: Clear operation states on chat change to prevent stale locks
    // Only clear if no generation is in progress (to avoid corrupting active operation)
    if (!operationState.generationInProgress) {
        operationState.extractionInProgress = false;
        operationState.retrievalInProgress = false;
    }
    ...
    // FIX: Always refresh UI on chat change (previously in separate duplicate handler)
    refreshAllUI();
    setStatus('ready');
}
...
async function onMessageReceived(messageId) {
    ...
    // FIX: Set extraction flag IMMEDIATELY after check to prevent race conditions
    // This prevents concurrent MESSAGE_RECEIVED events from both passing the check
    operationState.extractionInProgress = true;

    // FIX: Capture chat ID before any async operations to detect chat switch
    ...
    try {
        ...
        const data = getOpenVaultData();
        // FIX: Handle null data (context not available)
        if (!data) {
        ...
        // FIX: Check if chat changed during extraction - don't save to wrong chat
        ...
        // FIX: Only mark batch as extracted if events were actually created
        // This prevents permanently skipping batches that failed to parse
...
async function extractAllMessages() {
    ...
    setStatus('extracting');
    // Note: Using toastr directly here because we need the jQuery element for updates
...
function injectContext(contextText) {
    ...
    // FIX: Use safe wrapper with error handling
...
jQuery(() => {
    ...
    // FIX: Removed duplicate CHAT_CHANGED handler - UI refresh now happens in onChatChanged()
    // The handler registered in updateEventListeners() handles both cooldown AND UI refresh
});
